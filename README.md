...
- ✅ **单 Talk 音乐**：音乐只在当前 Talk 内生效，进入下一 Talk 后按规则停止
- ✅ **跨 Talk 背景音乐**：音乐可跨越多个 Talk 持续播放
- ✅ **背景型音乐**：支持自定义歌词字号、歌词颜色和循环方式
- ✅ **唱歌型音乐**：支持角色姓名、性别颜色、合唱和多角色演唱
- ✅ **扩展 LRC**：支持 `idN` 演唱槽、双语歌词和歌词句段编号
- ✅ **句段播放**：通过 `u/v` 从指定歌词开始，或播放到指定歌词结束
- ✅ **暂停与恢复**：暂停时记录位置并清理歌词，恢复时从原位置继续同步
- ✅ **400ms 保护**：防止玩家过快推进剧情而看不到插件演出
- ✅ **长按跳过**：强制完成型音乐可通过长按鼠标左键跳过
- ✅ **自动/快进控制**：不同指令可分别限制自动模式、手动推进和快进
- ✅ **双语浮动歌词**：原文和译文两行显示，并保持悬挂式左对齐
- ✅ **自动对比描边**：根据歌词附近背景亮度自动切换黑色或白色描边
- ✅ **固定歌词阴影**：所有浮动歌词带半透明黑色阴影
- ✅ **创意工坊资源包**：可直接读取其他 Mod 自带的音乐、歌词和 JSON
- ✅ **原版音乐外挂歌词**：无需替换原版音频，也可为原版音乐配置 LRC
- ✅ **正常游戏与编辑器兼容**：普通文本 Talk 和空文本 Talk 均支持
- ✅ **EFFECT 修复**：避免启动阶段组件销毁导致 `Effect Not Found`
- ✅ **精简日志**：只输出启动自检结果、1163 执行结果和具体错误
...
正常启动时应看到类似内容：
```text
[启动自检成功] BetterMusic 1.5.1 全流程自检通过；音乐条目=...，资源包=...。
```

  
若出现：
```text
[启动自检失败] BetterMusic 1.5.1 全流程自检未通过：...
```
...
---
# 十二、浮动歌词显示
BetterMusic 使用独立 TextMeshPro 浮动歌词层。
当前效果包括：
- 显示在 Talk 顶部区域；
- 角色名前缀与歌词正文分离；
- 支持单语和双语；
...
- 只在歌词实际显示时工作；
- 每 `0.35` 秒最多检测一次；
- 只采样歌词附近约 `64×4` 像素的小区域；
- 背景亮时使用黑色描边；
- 背景暗时使用白色描边；
- 亮度低于 `0.40` 才切换到白色描边；
- 亮度高于 `0.60` 才切换到黑色描边；
- 中间区域保持当前状态，避免频繁闪烁；
- 屏幕采样失败时保留默认黑色描边，不影响播放。
...
- 其他 Mod 深度修改 `NewTalkView`、`PreviewTalkView` 或 Talk 推进流程
- 其他 Mod直接控制原版 `AudioMgr channel=1`
- 其他 Mod 销毁或替换插件创建的独立 AudioSource
- 多个 BetterMusic 资源包使用相同音乐 ID
- 游戏版本更新导致目标方法签名变化
插件会定期检查关键 EFFECT 入口并尝试补装，但不能保证与所有修改同一底层方法的 Mod 同时兼容。
## 15.3 存档影响
...
│
├─ Lyrics/
│  ├─ FloatingLyricsOverlay.cs            # 浮动歌词 UI
│  ├─ LrcLine.cs                          # 单条歌词模型
│  ├─ LrcParser.cs                        # 扩展 LRC 解析
│  └─ LyricsContrastProbe.cs              # 背景亮度与自动描边
│
├─ Patches/
│  ├─ GenEffector1163Patch.cs             # 正常游戏 1163 工厂入口
│  ├─ TalkAdvancePatches.cs               # 手动、自动与快进控制
│  ├─ TalkLifecyclePatches.cs             # Talk 生命周期处理
│  ├─ HoldToSkipPatches.cs                # 长按跳过输入
│  └─ PreviewEffectTriggerPatches.cs      # 编辑器 Preview 触发入口
│
├─ Preview/
│  ├─ BetterMusicPreviewBridge.cs         # 编辑器 1163 执行桥
│  └─ PreviewTalkAccess.cs                 # 编辑器 Talk 数据访问
│
├─ Runtime/
│  ├─ BetterMusicController.cs            # 核心播放状态机
│  ├─ BetterMusicSession.cs               # 当前播放会话
│  ├─ MusicResolveContext.cs              # 音乐解析上下文
│  ├─ RuntimeTalkAccess.cs                # 正常游戏 Talk 访问
...
---
# 二十三、许可证与免责声明
参考https://github.com/white12666/StudentAgeEditorPlus，在此提出感谢。
